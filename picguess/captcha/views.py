...
def captcha_audio(request,key):
    if settings.CAPTCHA_FLITE_PATH:
        store = get_object_or_404(CaptchaStore,hashkey=key)
        text=store.challenge
        if 'picguess.captcha.helpers.math_challenge' == settings.CAPTCHA_CHALLENGE_FUNCT:
            text = text.replace('*','times').replace('-','minus')
        elif 'picguess.captcha.helpers.random_char_challenge' == settings.CAPTCHA_CHALLENGE_FUNCT:
            text = '.'.join(list(text))
